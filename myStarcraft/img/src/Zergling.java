...
class Zergling extends ZergUnit implements Runnable{
	static int cost=50;
	static int gascost=0;
	static int buildtime=25;
	static int attackspeed=200;
	Zergling(StarUI ui) {
		super(ui);
		name = "Zergling";
		energy = 50;
	...
	public void run(){
		while(true){
					...
					int rand;
						...
						while(true){ 
							if(ui.enemy.clan.attacker2VT.size()!=0){
								...
								((StarObject)(ui.enemy.clan.attacker2VT.elementAt(rand))).vt=ui.clan.attacker1VT;
								...
								((StarObject)(ui.enemy.clan.attacker1VT.elementAt(rand))).vt=ui.clan.attacker1VT;
			...
			case 3:
				...
				ui.enemy.clan.setUnit(ui.enemy.clan.attacker1VT.size(),ui.enemy.clan.attacker2VT.size(),ui.enemy.clan.workerVT.size());
				ui.enemy.U_unit.setText(String.valueOf(ui.enemy.clan.getUnit()));
				if(ui.enemy.U_name.getText()==target.name){ //보이는 화면이 일꾼일때
					ui.enemy.setTB(vt); // 제거
				}
				condi=(String)getCondition(0);
				if(ui.U_name.getText()==ui.clan.getAttacker1Name()){ //보이는 화면이 일꾼일때
					ui.setTB(ui.clan.attacker1VT); // 갱신
				}
				job=0;
				if(flag==false)
					break;
