...
class Tank extends TerranUnit implements Runnable {
	static int cost=100;
	...
	public void run(){
		while(true){
			...
			case 1:
				...
				while(true){ 
					if(ui.enemy.clan.attacker2VT.size()!=0){
						...
						if(ui.enemy.U_name.getText()==ui.enemy.clan.getAttacker2Name()){ 
			...
			case 3:
				...
				condi=(String)getCondition(0);
				if(ui.U_name.getText()==ui.clan.getAttacker2Name()){ //보이는 화면이 일꾼일때
